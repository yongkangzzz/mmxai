...
class MultiModalExplanation(object):
    ...
    def get_explanation(self, label, num_features=10):
        ...
        txt_exp_list = self.as_list(label)[:n_txt_exp]
        temp, mask = self.get_image_and_mask(label, num_features=n_img_exp)

        return this_exp, readable_exp, txt_exp_list, temp, mask
...
class LimeMultimodalExplainer(object):
    ...
    def explain_instance(self, classifier_fn, n_samples, top_labels=2):
        ...
        data, labels, distances, n_txt_features, n_img_features,\
            segments, domain_mapper = self.data_labels(n_samples, classifier_fn)
        num_features = data.shape[1]

        print("text features:", n_txt_features, "image features:", n_img_features, ",data:", data.shape)
        ...
        for label in top:
            (ret_exp.intercept[label],
             ...
             ret_exp.local_pred[label]) = self.base.explain_instance_with_data(
                data, labels, distances, label, num_features, n_txt_features, n_img_features,
                # feature_selection=self.feature_selection, )
                feature_selection=None)

            '''
            unsorted_exp_txt = list(ret_exp.unsorted_weights[label])[:n_txt_features]
            unsorted_exp_img = list(ret_exp.unsorted_weights[label])[n_txt_features:]
            ret_exp.local_exp_txt[label] = sorted(unsorted_exp_txt, key=lambda x: np.abs(x[1]), reverse=True)
            ret_exp.local_exp_img[label] = sorted(unsorted_exp_img, key=lambda x: np.abs(x[1]), reverse=True)
            '''
            # split local explanation into text and image features

        return ret_exp

    def data_labels(self, num_samples, classifier_fn, object_detection=False):
        '''
        ...
        data = np.vstack((data, np.concatenate((orig_txt_f, orig_img_f))))
        labels.extend(classifier_fn([self.image], [self.text]))

        '''4. compute distance# distances[:, :(doc_size-1)] *= 100'''

        labels = np.array(labels, dtype="object")
        # Naive - get relative importance of text and image
        '''dummy_img = np.zeros_like(self.image)
        img_shape = self.image.shape
        imgs_dummy = []
        imgs_dummy.append(self.image)
        imgs_dummy.append(dummy_img)
        txts_dummy = []
        txts_dummy.append("")
        txts_dummy.append(self.text)

        # first item: original image, second: original text
        labels_dummy = np.array(classifier_fn(imgs_dummy, txts_dummy))'''

        # Modify MMF source code to zero out image / text attributes
        dummy_label_image = np.array(classifier_fn([self.image], [self.text], zero_text=True))
        dummy_label_text = np.array(classifier_fn([self.image], [self.text], zero_image=True))
        delta_txt = abs(labels[-1][0] - dummy_label_image[0][0])
        delta_img = abs(labels[-1][0] - dummy_label_text[0][0])

        ratio_txt_img = min(100, delta_txt/delta_img)

        print("labels", labels.shape, labels)
        print("labels dummy shape, should be 1x2", dummy_label_image.shape, dummy_label_image)
        print(delta_txt, delta_img, ratio_txt_img)
        ...
        print("distances image", distances_img)
        print("distances txt", distances_txt)

        return data, labels, distances, doc_size, n_img_features, segments, domain_mapper
