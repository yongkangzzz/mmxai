...
from lime4mmf.lime_base import LimeBase
...
class MultiModalExplanation(object):
    ...
    def get_explanation(self, label, num_features=10, which_exp='positive'):
        ...
        return this_exp, readable_exp, txt_list, temp, mask
    
    def get_txt_img_ratio(self):
        img_percentage = 1 / (1 + self.txt_img_ratio)
        txt_percentage = 1 - img_percentage
        words = f"For this prediction, the relative importance of " \
                f"text and image inputs are respectively {round(100*txt_percentage, 2)}% " \
...
class LimeMultimodalExplainer(object):
    ...
    def __init__(self, image, text, model, kernel_width=.25, kernel=None,
    ...
    def explain_instance(self, classifier_fn, n_samples, top_labels=2):
        ...
        data, labels, distances, n_txt_features, n_img_features,\
            segments, domain_mapper, n_detection_features, \
            detection_label, ratio_txt_img = self.data_labels(n_samples, classifier_fn)
        ...
        for label in top:
            (ret_exp.intercept[label],
             ...
             ret_exp.local_pred[label]) = self.base.explain_instance_with_data(
                data, labels, distances, label, num_features, n_txt_features, 
                n_img_features, n_detection_features, ratio_txt_img, 
    ...
    def data_labels(self, num_samples, classifier_fn, detection=False):
        ...
        try:
            ...
            try:
                label_text_out = np.array(classifier_fn([self.image], [self.text], zero_text=True))  # zero out text
                label_image_out = np.array(classifier_fn([self.image], [self.text], zero_image=True))  # zero out image
            except:
                label_text_out = np.array(classifier_fn([self.image], [dummy_text]))
                label_image_out = np.array(classifier_fn([dummy_image], [self.text]))

            delta_txt = abs(labels[-1][0] - label_text_out[0][0])
            delta_img = abs(labels[-1][0] - label_image_out[1][0])
            ratio_txt_img = max(min(10, delta_txt / delta_img), 0.1)
