...
def multi_extremal_perturbation(model,
    ...
    for t in range(max_iter):
        ...
        # DUAL_VARIANT is excluded due to torch dimension mismatch
        ...
        y = model(torch.squeeze(x, 0), input_text)  # !!! Changed by Chenyu !!!
    ...
    text_explanation_presentation(
        input_text, torch.squeeze(x, 0), image_path, model, show_img=show_text_result)
...
def text_explainer(string, img_tensor, classifier_fn):
    ...
    for i in range(len(masked_text)):
        temp = classifier_fn(img_tensor, masked_text[i]).detach().cpu().numpy()
...
def text_explanation_presentation(input_text, image_tensor, image_path, model, show_img=True):
    """
    args :
        input_text : original text
